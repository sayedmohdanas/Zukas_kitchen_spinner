...
/**
 * Normalizes mobile number into standard international format (+91XXXXXXXXXX)
 */
...
/**
 * Server-side coupon code generator
 */
...
/**
 * Server-Authoritative Cloud Function: spinWheel
 * Enforces atomic cooldown checks, server-side weighted prize selection,
 * server timestamping, and server coupon generation.
 */
exports.spinWheel = onCall({ cors: true }, async (request) => {
  const mobileInput = request.data ? request.data.mobile : null;
  const normalizedMobile = normalizeMobile(mobileInput);

  // Execute inside an atomic Firestore transaction to prevent race conditions
  return await db.runTransaction(async (transaction) => {
    ...
    if (!campaignConfig.enabled) {
      throw new HttpsError("failed-precondition", "Spin & Win is currently closed. Please check back soon! ❤️");
    }
    ...
    if (!existingSpinsSnap.empty) {
      ...
      if (!campaignConfig.repeatEnabled) {
        throw new HttpsError("already-exists", "You have already used your Spin & Win chance.");
      }
    ...
    // 3. Read active prizes from Firestore collection or use SERVER_PRIZES fallback
    ...
    // 4. Server-Side Weighted Prize Selection
    ...
    // Determine prizeIndex relative to standard 6 prizes array order
    let targetIndex = SERVER_PRIZES.findIndex((p) => p.id === selectedPrize.id);
    if (targetIndex === -1) targetIndex = 0;

    // 5. Server-Side Coupon Generation
    const couponCode = generateServerCoupon(selectedPrize);

    // 6. Write new spin document inside transaction with trusted server timestamp
